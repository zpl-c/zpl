...
void zpl_key_state_update(zpl_key_state *s, zpl_b32 is_down) {
    ...
    zpl_internal ZPL_ALWAYS_INLINE zpl_f32 zpl__process_xinput_stick_value(zpl_i16 value, zpl_i16 dead_zone_threshold) {
        zpl_f32 result = 0;
    ...
    zpl_b32 zpl_platform_init_with_opengl(zpl_platform *p, char const *window_title, zpl_i32 width, zpl_i32 height,
...
zpl_video_mode zpl_set_video_mode(zpl_i32 width, zpl_i32 height, zpl_i32 bits_per_pixel) {
    zpl_video_mode m;
    m.width = width;
    m.height = height;
...
zpl_b32 zpl_video_mode_is_valid(zpl_video_mode mode) {
