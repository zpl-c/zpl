...
int main() {
    zpl_thread_pool p={0}, q={0};
    zpl_jobs_init_with_limit(&p, zpl_heap(), CORES, JOBS);
    zpl_jobs_init_with_limit(&q, zpl_heap(), CORES, JOBS);
    ...
    while (counter < N) {
        if (zpl_jobs_empty(&p) && zpl_jobs_empty(&q)) {
            counter++;
            zpl_printf("Iterations left: %d.      \r", (N-counter));
            for (int i=0; i<NL; i++) {
                zpl_jobs_enqueue(&p, work_low, 0);
            }
            for (int i=0; i<NH; i++) {
                zpl_jobs_enqueue(&q, work_high, 0);
            }
        } else {
