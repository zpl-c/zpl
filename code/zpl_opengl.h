...
#define zplgl_vert_ptr_aa_u16n(index, element_count, Type, var_name) do { \
    ...
    ZPLGL_DEF zpl_u32 zplgl_make_sampler(zpl_u32 min_filter, zpl_u32 max_filter, zpl_u32 s_wrap, zpl_u32 t_wrap);
    ...
    ZPLGL_DEF zpl_u32 zplgl_make_vbo(void const *data, zpl_isize size, zpl_i32 usage_hint);
    ZPLGL_DEF zpl_u32 zplgl_make_ebo(void const *data, zpl_isize size, zpl_i32 usage_hint);
    ZPLGL_DEF zpl_u32 zplgl_make_ssbo(void const *data, zpl_isize size, zpl_i32 ssbo_usage_hint);
    ZPLGL_DEF zpl_u32 zplgl_make_vao(void);
    
    ZPLGL_DEF zplgl_tbo zplgl_make_tbo(zplgl_buffer_data_type data_type, zpl_i32 channel_count, void const *data, zpl_isize size, zpl_i32 usage_hint);

    ZPLGL_DEF void zplgl_vbo_copy(zpl_u32 vbo_handle, void *const data, zpl_isize size, zpl_isize offset);
    ZPLGL_DEF void zplgl_ebo_copy(zpl_u32 ebo_handle, void *const data, zpl_isize size, zpl_isize offset);
    ZPLGL_DEF void zplgl_ssbo_copy(zpl_u32 ssbo_handle, void *const data, zpl_isize size, zpl_isize offset);
    ZPLGL_DEF void zplgl_tbo_copy(zplgl_tbo tbo, void *const data, zpl_isize size, zpl_isize offset);



    ZPLGL_DEF void zplgl_bind_vbo(zpl_u32 vbo_handle);
    ZPLGL_DEF void zplgl_bind_ebo(zpl_u32 ebo_handle);
    ZPLGL_DEF void zplgl_bind_ssbo(zpl_u32 ssbo_handle);
    ZPLGL_DEF void zplgl_bind_tbo(zplgl_tbo tbo, zpl_i32 sampler_handle, zpl_i32 tex_unit);

    // NOTE(bill): access = GL_WRITE_ONLY, etc.
    ZPLGL_DEF void *zplgl_map_vbo(zpl_u32 vbo_handle, zpl_i32 access);
    ZPLGL_DEF void *zplgl_map_ebo(zpl_u32 ebo_handle, zpl_i32 access);
    ZPLGL_DEF void *zplgl_map_ssbo(zpl_u32 ssbo_handle, zpl_i32 access);
...
#define ZPLGL_SHADER_FILE_EXTENSIONS_DEFINED
    ...
    ZPLGL_DEF zplgl_shader_error zplgl_load_shader_from_file(zplgl_shader *s, zpl_u32 type_bits, char const *filename);
    ZPLGL_DEF zplgl_shader_error zplgl_load_shader_from_memory_vf(zplgl_shader *s, char const *vert_source, char const *frag_source);
    ZPLGL_DEF zplgl_shader_error zplgl_load_shader_from_memory_vfg(zplgl_shader *s, char const *vert_source, char const *frag_source, char const *geom_source);

    ZPLGL_DEF void zplgl_destroy_shader(zplgl_shader *shader);
    ZPLGL_DEF zpl_b32  zplgl_has_shader_changed(zplgl_shader *shader);
    ZPLGL_DEF zpl_b32  zplgl_reload_shader(zplgl_shader *shader); // TODO(bill): Return an error code?
    ZPLGL_DEF void zplgl_use_shader(zplgl_shader *shader);
    ZPLGL_DEF zpl_b32  zplgl_is_shader_in_use(zplgl_shader *shader);

    ZPLGL_DEF zpl_i32 zplgl_get_uniform(zplgl_shader *shader, char const *name);
    ZPLGL_DEF zpl_i32 zplgl_get_storage_block(zplgl_shader *s, char const *name);
    ZPLGL_DEF void zplgl_bind_ssbo_storage_block(zplgl_shader *s, zpl_i32 binding_point, zpl_i32 block_index, zpl_i32 ssbo_handle);

    ZPLGL_DEF void zplgl_set_uniform_int(zplgl_shader *s, char const *name, zpl_i32 i);
    ZPLGL_DEF void zplgl_set_uniform_float(zplgl_shader *s, char const *name, zpl_f32 f);
    ZPLGL_DEF void zplgl_set_uniform_vec2(zplgl_shader *s, char const *name, zpl_f32 const *v);
    ZPLGL_DEF void zplgl_set_uniform_vec3(zplgl_shader *s, char const *name, zpl_f32 const *v);
    ZPLGL_DEF void zplgl_set_uniform_vec4(zplgl_shader *s, char const *name, zpl_f32 const *v);
    ZPLGL_DEF void zplgl_set_uniform_mat4(zplgl_shader *s, char const *name, zpl_f32 const *m);
    ZPLGL_DEF void zplgl_set_uniform_mat4_count(zplgl_shader *s, char const *name, zpl_f32 const *m, zpl_isize count);
    ...
    ZPLGL_DEF zpl_b32  zplgl_load_texture2d_from_file(zplgl_texture *texture, zpl_b32 flip_vertically, char const *filename, ...);
    ZPLGL_DEF zpl_b32  zplgl_load_texture2d_from_memory(zplgl_texture *texture, void const *data, zpl_i32 width, zpl_i32 height, zpl_i32 channel_count);
    ZPLGL_DEF zpl_b32  zplgl_init_texture2d_coloured(zplgl_texture *texture, zplgl_color colour);
    ZPLGL_DEF void zplgl_destroy_texture(zplgl_texture *texture);

    ZPLGL_DEF void zplgl_bind_texture2d(zplgl_texture const *texture, zpl_u32 position, zpl_u32 sampler);
...
#define ZPLGL_MAX_RENDER_COLOUR_BUFFERS 16
    ...
    ZPLGL_DEF zpl_b32  zplgl_init_render_buffer(zplgl_render_buffer *rb, zpl_i32 width, zpl_i32 height, zpl_i32 channel_count);
    ...
    ZPLGL_DEF void zplgl_render_to_screen(zpl_i32 width, zpl_i32 height);
    ...
    ZPLGL_DEF zplgl_font *zplgl_load_font_from_file(zplgl_font_cache *fc, char const *ttf_filename, zpl_f32 font_size);
    ZPLGL_DEF zplgl_font *zplgl_get_font_only_from_cache(zplgl_font_cache *fc, char const *ttf_filename, zpl_f32 font_size);
    ZPLGL_DEF zplgl_font *zplgl_cache_font(zplgl_font_cache *fc, char const *ttf_filename, zpl_f32 font_size);


    ZPLGL_DEF zpl_b32            zplgl_get_packed_font_dim(zplgl_font_cache *cache, zplgl_font_cached_ttf *ttf, zpl_i32 *width, zpl_i32 *height);
    ZPLGL_DEF zplgl_glyph_info  *zplgl_get_glyph_info(zplgl_font *font, zpl_char32 codepoint, zpl_isize *out_index);
    ZPLGL_DEF zpl_f32            zplgl_get_font_kerning_from_glyph_indices(zplgl_font *font, zpl_isize left_index, zpl_isize right_index);
    ZPLGL_DEF void               zplgl_get_string_dimensions(zplgl_font *font, char const *str, zpl_f32 *out_width, zpl_f32 *out_height);
    ZPLGL_DEF zpl_f32            zplgl_get_sub_string_width(zplgl_font *font, char const *str, zpl_isize char_count);
    ZPLGL_DEF zpl_i32            zplgl_get_wrapped_line_count(zplgl_font *font, char const *str, zpl_isize max_len, zpl_isize max_width);
    ZPLGL_DEF zpl_f32            zplgl_get_string_width(zplgl_font *font, char const *str, zpl_isize max_len);
...
#define ZPLGL_TAB_CHARACTER_WIDTH 4
    ...
    ZPLGL_DEF void zplgl_bs_draw_rect(zplgl_basic_state *bs, zpl_f32 x, zpl_f32 y, zpl_f32 w, zpl_f32 h, zplgl_color col);
    ZPLGL_DEF void zplgl_bs_draw_rect_outline(zplgl_basic_state *bs, zpl_f32 x, zpl_f32 y, zpl_f32 w, zpl_f32 h, zplgl_color col, zpl_f32 thickness);
    ...
    ZPLGL_DEF void zplgl_bs_draw_line(zplgl_basic_state *bs, zpl_f32 x0, zpl_f32 y0, zpl_f32 x1, zpl_f32 y1, zplgl_color col, zpl_f32 thickness);

    ZPLGL_DEF void zplgl_bs_draw_elliptical_arc(zplgl_basic_state *bs, zpl_f32 x, zpl_f32 y, zpl_f32 radius_a, zpl_f32 radius_b, zpl_f32 min_angle, zpl_f32 max_angle, zplgl_color col);
    ZPLGL_DEF void zplgl_bs_draw_elliptical_arc_outline(zplgl_basic_state *bs, zpl_f32 x, zpl_f32 y, zpl_f32 radius_a, zpl_f32 radius_b,
        zpl_f32 min_angle, zpl_f32 max_angle, zplgl_color col, zpl_f32 thickness);

    ZPLGL_DEF void zplgl_bs_draw_circle(zplgl_basic_state *bs, zpl_f32 x, zpl_f32 y, zpl_f32 radius, zplgl_color col);
    ZPLGL_DEF void zplgl_bs_draw_circle_outline(zplgl_basic_state *bs, zpl_f32 x, zpl_f32 y, zpl_f32 radius, zplgl_color col, zpl_f32 thickness);
    ...
    ZPLGL_DEF void zplgl_bs_draw_rounded_rect_corners(zplgl_basic_state *bs, zpl_f32 x, zpl_f32 y, zpl_f32 w, zpl_f32 h, zpl_f32 roundness, zplgl_color col, zpl_u32 corners);
    ZPLGL_DEF void zplgl_bs_draw_rounded_rect(zplgl_basic_state *bs, zpl_f32 x, zpl_f32 y, zpl_f32 w, zpl_f32 h, zpl_f32 roundness, zplgl_color col);

    ZPLGL_DEF void zplgl_bs_draw_rounded_rect_corners_outline(zplgl_basic_state *bs, zpl_f32 x, zpl_f32 y, zpl_f32 w, zpl_f32 h, zpl_f32 roundness, zplgl_color col, zpl_f32 thickness, zpl_u32 corners);
    ZPLGL_DEF void zplgl_bs_draw_rounded_rect_outline(zplgl_basic_state *bs, zpl_f32 x, zpl_f32 y, zpl_f32 w, zpl_f32 h, zpl_f32 roundness, zplgl_color col, zpl_f32 thickness);


#if !defined(ZPLGL_NO_FONTS)
    ZPLGL_DEF zpl_isize zplgl_bs_draw_substring(zplgl_basic_state *bs, zplgl_font *font, zpl_f32 x, zpl_f32 y, zplgl_color col, char const *str, zpl_isize len);
    ZPLGL_DEF zpl_isize zplgl_bs_draw_string(zplgl_basic_state *bs, zplgl_font *font, zpl_f32 x, zpl_f32 y, zplgl_color col, char const *fmt, ...);
    ZPLGL_DEF zpl_isize zplgl_bs_draw_string_va(zplgl_basic_state *bs, zplgl_font *font, zpl_f32 x, zpl_f32 y, zplgl_color col, char const *fmt, va_list va);
#endif
...
zplgl_font * zplgl_cache_font(zplgl_font_cache *fc, char const *ttf_filename, zpl_f32 font_size) {
    ...
    if (!fc->font_char_list) {
        ...
        fc->codepoints = cast(zpl_char32 *)             zplgl_malloc(zpl_size_of(zpl_char32)        * codepoint_count);
        ...
        if (!*ttf_cache) {
            ...
            *ttf_cache = cast(zplgl_font_cached_ttf *)zplgl_malloc(zpl_size_of(zplgl_font_cached_ttf));
            ...
            if (zpl_file_open(&file, ttf_filename) == ZPL_FILE_ERROR_NONE) {
                ...
                (*ttf_cache)->ttf = cast(zpl_u8 *)zplgl_malloc(len);
    ...
    { // NOTE(bill): Figure out smallest non-square power of 2 texture size
        ...
        if (zplgl_get_packed_font_dim(fc, ttf, &w, &h)) {
            ...
            if (!f->glyph_map || !f->glyphs) {
                ...
                px = cast(zpl_u8 *)zplgl_malloc(w * h);
    ...
    if (roundness == 0 || corners == 0) {
        ...
        if (corners & 1) {
            for (i = 0; i < 6; i++) {
                ...
                zpl_f32 a = zplgl_lerp(0.5f*ZPLGL_TAU, 0.75f*ZPLGL_TAU, t);
                zpl_f32 c = zplgl_cos(a);
                zpl_f32 s = zplgl_sin(a);
        ...
        if (corners & 2) {
            for (i = 0; i < 6; i++) {
                zpl_f32 t = cast(zpl_f32)i / 5.0f;
                zpl_f32 a = zplgl_lerp(0.75f*ZPLGL_TAU, 1.00f*ZPLGL_TAU, t);
                zpl_f32 c = zplgl_cos(a);
                zpl_f32 s = zplgl_sin(a);
        ...
        if (corners & 4) {
            for (i = 0; i < 6; i++) {
                zpl_f32 t = cast(zpl_f32)i / 5.0f;
                zpl_f32 a = zplgl_lerp(0.00f*ZPLGL_TAU, 0.25f*ZPLGL_TAU, t);
                zpl_f32 c = zplgl_cos(a);
                zpl_f32 s = zplgl_sin(a);
                bs->vertices[vc].x = x + w - roundness + c * roundness;
        ...
        if (corners & 8) {
            for (i = 0; i < 6; i++) {
                zpl_f32 t = cast(zpl_f32)i / 5.0f;
                zpl_f32 a = zplgl_lerp(0.25f*ZPLGL_TAU, 0.50f*ZPLGL_TAU, t);
                zpl_f32 c = zplgl_cos(a);
                zpl_f32 s = zplgl_sin(a);
                bs->vertices[vc].x = x + roundness + c * roundness;
                bs->vertices[vc].y = y + h - roundness + s * roundness;
