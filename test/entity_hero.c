...
typedef struct {
    ...
    armor_piece_t *Torso;
    armor_piece_t *Legs;
} ZPL_ENT_COMP_DECLARE(armor);

typedef struct {
    zpl_ent_pool entities;
...
ZPL_ENT_COMP_DEFINE(armor);
////

void world_init(world_t *w, zpl_allocator_t alloc, isize count) {
    zpl_ent_init(&w->entities, alloc, count);
...
zpl_entity_t hero_spawn(world_t *world, char *const name, i32 age, position_t pos, f32 strength_factor) {
    zpl_entity_t hero = zpl_ent_create(&world->entities);
...
void hero_kill(world_t *world, zpl_entity_t hero) {
...
int main(void) {
    ...
    {
        for (isize i = 0; i < 5; ++i) {
            zpl_entity_t hero = hero_spawn(&world, names[i], (25 + (i%3)*4), (position_t){ i, i*2, i*i }, i % 4);
